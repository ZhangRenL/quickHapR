---
title: "quickHapR Usage"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{quickHapR}
...
hap = get_hap(vcf,                 # import_vcf() 导入的vcfR
              ...
              startPOS = 4000,   # Numeric, 起始位置，通过位置对vcf信息进行筛选
              endPOS = 9000,     # Numeric, 终止位置，通过位置对vcf信息进行筛选
              na.drop = TRUE,
              hyb_remove = TRUE)
...
plotGeneStructure(gff,                # gff注释信息
                  ...
                  startPOS = start(range(gff)),  # 基因结构示意图的起始位点
                  endPOS = end(range(gff)),    # 基因结构示意图的终止位置
                  type = "pin",       # SNP类型
                  cex = 1,            # circle大小
...
res = hapVsPheno(hap,        # data.frame:第一列与最后一列分别固定为HAP和Accession，中间列为位置及对应的基因型
                 ...
                 phenoName = colnames(pheno)[1],  # 本次分析中使用的表型名称
                 hapPrefix = "H",            # 单倍型编号的前缀
                 geneID = "Seita.1G000000",  # 基因ID， 作为表头信息
                 mergeFigs = FALSE,    # 是否将两图融合
                 minAcc = 3)       # 需要分析的单倍型包含的数据量最小值
